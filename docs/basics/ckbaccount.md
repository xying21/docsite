...
For more information about how to create CKB accounts, see <Link to={useBaseUrl('/docs/develop/createaccount')}>Create Accounts</Link>.
...
Accounts on Testnet can request CKB capacity on [https://faucet.nervos.org](https://faucet.nervos.org/). Accounts on DEV chain can obtain CKB capacity by mining blocks as miners. For more information about getting CKB capacity, see **<Link to={useBaseUrl('/docs/develop/createaccount#step-5-get-ckb-capacity-for-the-account-of-alice')}>step 5</Link>** of <Link to={useBaseUrl('/docs/develop/createaccount#create-the-accounts-by-using-ckb-cli')}>Create the accounts by Using ckb-cli</Link>.
