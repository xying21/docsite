...
In this guide, we will prepare two CKB accounts, Alice and Bob, that will be used in the <var>accounts.ts</var> file in the later <Link to={useBaseUrl('/docs/tools/lumos/preparation/hellolumos')}>Hello Lumos</Link> example. For more information about the concepts of CKB accounts, CKB capacity, and the methods to create accounts, see <Link to={useBaseUrl('/docs/basics/ckbaccount')}>CKB Accounts and Capacity</Link>.

## Prerequisites 

The following prerequisites apply for creating the accounts:

- The CKB node is installed and running on DEV chain. For more information, see <Link to={useBaseUrl('/docs/tools/lumos/preparation/setupsystem#install-a-ckb-node-on-dev-chain-by-using-tippy')}>Install a CKB node on DEV Chain by Using Tippy</Link>. 

## Create the Accounts by Using ckb-cli

### Step 1. Download the CKB pre-built installer package.

The CKB pre-built installer package includes the ckb-cli tool. For more information, see <Link to={useBaseUrl('/docs/basics/ckbnode#step-1-download-the-ckb-pre-built-installer-package')}>Download the CKB Pre-built Installer Package</Link>.
...
For more information about CKB capacity, see <Link to={useBaseUrl('/docs/basics/ckbaccount#ckb-capacity-of-an-account')}>CKB Capacity of an Account</Link>.
